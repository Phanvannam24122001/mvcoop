...
<?php
	...
	class product
	{
		...
		public function insert_product($data,$files){
			...
			$type2 = mysqli_real_escape_string($this->db->link, $data['type2']);
			...
			if($productName=="" || $brand=="" || $category=="" || $product_desc=="" || $price=="" || $type=="" || $file_name ==""|| $type1==""|| $type2==""){
				$alert = "<span class='success'> Không Được Để Trống</span>";
				return $alert;
			}else{
				move_uploaded_file($file_temp,$uploaded_image);
				$query = "INSERT INTO tbl_product(productName,brandId,catId,product_desc,price,type,image,type1,type2) VALUES('$productName','$brand','$category','$product_desc','$price','$type','$unique_image','$type1','$type2')";
		...
		public function update_product($data,$files,$id){
			...
			if($productName=="" || $brand=="" || $category=="" || $product_desc=="" || $price=="" || $type==""|| $type1==""|| $type2==""){
		...
		public function get_product_by_cat($id){
			$query = "SELECT * FROM tbl_product WHERE catId order by catId desc LIMIT 8";
			$result = $this->db->select($query);
			return $result;
		}
